...
static void test_legacy_non_control_and_builders(void)
{
  ...
  CHECK(ML307_MqttBuildSslConfig(out, sizeof(out), 0U, 1U, 2U) == ML307_RESULT_OK);
  CHECK(!strcmp(out, "AT+MQTTCFG=\"ssl\",0,1,2\r\n"));
  CHECK(ML307_MqttBuildSslConfig(out, sizeof(out), 6U, 1U, 0U) ==
        ML307_RESULT_INVALID_VALUE);
  CHECK(ML307_MqttBuildSslConfig(out, sizeof(out), 0U, 2U, 0U) ==
        ML307_RESULT_INVALID_VALUE);
  CHECK(ML307_MqttBuildSslConfig(out, sizeof(out), 0U, 1U, 6U) ==
        ML307_RESULT_INVALID_VALUE);
}
